...
class NewsEventMonitor:
    ...
    def update(self, article: NewsArticle):
        ...
        while sims[sort_index[idx]] > self.sim_threshold:
            ...
            article_txt = article.get_text()
            event_txt = event.articles[0].get_text()
            
            with torch.no_grad():
                result = self.model([article_txt], [event_txt])
                #print(result)
            
            
            if result > 0.7 and time_diff <= self.time_threshold:
    ...
    def get_dataframe(self):
        ...
        self.assign_events_to_articles()
        events = self.get_events()
        for event in events:
            for article in event.articles:
                df.loc[len(df)] = article.get_dfList_row()
        return df
